...
class _PlanerScreenState extends State<PlanerScreen> {
  ...
  void _calculate() {
    Fehlerbericht.log(
        "Berechnung gestartet: Temp $_temp°C, Zielzeit $_hours h");
    final hours = _hours;
    if (hours <= 0) {
    ...
    if (_temp <= 6) {
      ...
      tempAdvice =
          'Extrem langsam. Nur für Pausen geeignet. Starter erst auf Raumtemperatur bringen.';
    } else if (_temp <= 14) {
      tempLabel = 'Sehr kalt';
      tempAdvice =
          'Langsam. Starter an wärmeren Ort stellen (Backofen mit Lampe, ~25°C).';
    } else if (_temp <= 20) {
      ...
      tempAdvice =
          'Schnell. Regelmäßig prüfen — Höhepunkt kommt und geht schnell.';
    } else if (_temp <= 35) {
      tempLabel = 'Heiß — Vorsicht';
      tempAdvice =
          'Sehr schnell. Kaltes Wasser (8–12°C) nutzen. Gut im Auge behalten.';
    } else if (_temp <= 38) {
      tempLabel = '⚠️ Kritisch heiß';
      tempAdvice =
          'Am Limit! Nach dem Füttern sofort in den Kühlschrank (4–6°C). Dort langsam fermentieren lassen.';
    } else {
      tempLabel = '☠️ Zu heiß!';
      tempAdvice =
          'Ab 38°C sterben Milchsäurebakterien! Starter SOFORT kühlen. Backen verschieben.';
    }
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(children: [
          ...
          SizedBox(width: 8),
          Flexible(
              child: Text('Zu wenig Anstellgut',
                  style: TextStyle(
  ...
  Future<void> _scheduleNotifications(PlanerResult r) async {
    if (kIsWeb) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('ℹ️ Erinnerungen nicht im Browser verfügbar')),
      );
      return;
    }
    ...
    await scheduleAt(1, r.dHalf, '🫧 Sauerteig prüfen',
    ...
    await scheduleAt(2, preWarning, '⏰ Sauerteig fast fertig!',
        'In ~15 Minuten ist der Höhepunkt erreicht. Bereit machen!');

    await scheduleAt(3, r.dPeak, '🎯 HÖHEPUNKT — Sauerteig verwenden!',
  ...
  Future<void> _exportICS() async {
    ...
    String makeEvent(
        DateTime start, DateTime end, String summary, String desc) {
      return 'BEGIN:VEVENT\r\n'
    ...
    ics.write(makeEvent(
        r.dFeed,
        r.dFeed.add(const Duration(minutes: 10)),
        '🍞 Sauerteig füttern',
        'Verhältnis ${r.ratio}\nAnstellgut: ${r.anstellgut}g\nWasser: ${r.wasser}g\nMehl: ${r.mehl}g'));
    ics.write(makeEvent(
        r.dHalf,
        r.dHalf.add(const Duration(minutes: 10)),
        '🫧 Erste Aktivität prüfen',
        'Bläschen sichtbar?\nNoch ${formatH(r.estTime * 0.5)} bis Höhepunkt.'));
    ics.write(makeEvent(
        r.dPeak,
        r.dPeak.add(const Duration(minutes: 15)),
      ...
      await Share.shareXFiles(
        [
          XFile.fromData(bytes,
              name: 'Sauerteig_Zeitplan.ics', mimeType: 'text/calendar')
        ],
  ...
  Future<void> _exportPDF() async {
        ...
        children: [
          ...
          pw.Container(
            ...
            child: pw.Text('🍞 Sauerteig Zeitplan',
                style: pw.TextStyle(
                    fontSize: 20, color: gold, fontWeight: pw.FontWeight.bold)),
          ),
          ...
          _pdfTimeline(
              fmt.format(r.dFeed),
              'Jetzt — Füttern',
              'Anstellgut ${r.anstellgut}g + Wasser ${r.wasser}g + Mehl ${r.mehl}g',
              gold),
          ...
          pw.Text(r.tempLabel,
              style: pw.TextStyle(
                  fontWeight: pw.FontWeight.bold, fontSize: 9, color: dark)),
          pw.SizedBox(height: 4),
          pw.Text(r.tempAdvice,
    ...
    await Printing.sharePdf(
        bytes: await doc.save(), filename: 'Sauerteig_Zeitplan.pdf');
  }
  ...
  pw.Widget _pdfBox(String val, String lbl, PdfColor gold, pw.Context ctx) =>
      pw.Expanded(
        ...
        child: pw.Column(children: [
          pw.Text(val,
              style: pw.TextStyle(
                  color: gold, fontWeight: pw.FontWeight.bold, fontSize: 13),
  ...
  pw.Widget _pdfSection(String title, PdfColor gold) => pw.Container(
        ...
        child: pw.Text(title,
            style: pw.TextStyle(
                color: gold, fontWeight: pw.FontWeight.bold, fontSize: 9)),
      );

  pw.Widget _pdfRow(String label, String val, PdfColor color,
          {bool bold = false}) =>
      pw.Padding(
  ...
  pw.Widget _pdfTimeline(
          String time, String title, String desc, PdfColor color) =>
      pw.Padding(
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _Card(
        ...
        Row(children: [
          Expanded(
              child: _SummaryBox(
                  value: '${r.temp.toInt()}°C',
                  label: r.tempLabel,
                  color: tempColor(r.temp))),
          const SizedBox(width: 8),
          Expanded(
              child: _SummaryBox(
                  value: r.ratio, label: 'Verhältnis', color: AppColors.gold2)),
          const SizedBox(width: 8),
          Expanded(
              child: _SummaryBox(
                  value: '~${formatH(r.estTime)}',
                  label: 'bis Peak',
        ...
        if (r.fExact != r.factor) ...[
          ...
          _InfoBox(
            ...
            text:
                '🔢 Exaktes Verhältnis: 1 / ${r.fExact.toStringAsFixed(1)} / ${r.fExact.toStringAsFixed(1)}'
      ...
      _Card(
        ...
        _TimelineItem(
          ...
          title: 'Füttern',
          desc:
              'Anstellgut ${r.anstellgut}g + Wasser ${r.wasser}g + Mehl ${r.mehl}g verrühren. Höhe markieren.',
          isNow: true,
        ),
        _TimelineItem(
        ...
        _TimelineItem(
          ...
          desc:
              'Verdoppelt. ${r.forRecipe}g abnehmen → in den Teig. Float-Test!',
          isHighlight: true,
        ),
        _TimelineItem(
          time: 'danach',
          ...
          desc:
              '${r.leftover}g als neues Anstellgut. ${r.temp > 28 ? 'Sofort in den Kühlschrank.' : 'Kühlschrank oder weiterführen.'}',
        ),
        const SizedBox(height: 10),
        const _InfoBox(
          ...
          text:
              '🧪 Float-Test: 1 TL Starter ins Wasser — schwimmt er? → Bereit ✅   Sinkt er? → Noch warten.',
        ),
      ])),
      ...
      _Card(
        ...
        Text(r.tempAdvice,
            style: const TextStyle(
                color: AppColors.text2, fontSize: 14, height: 1.6)),
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            style: const TextStyle(
                fontFamily: 'monospace', fontSize: 18, color: AppColors.text),
...
class _RecipeTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Table(
        ...
        children: [
          _tableHeader(),
          _tableRow(
              'Anstellgut', '${r.anstellgut} g', 'aus vorhandenem Starter'),
          _tableRow(
              'Wasser',
              '${r.wasser} g',
              r.temp >= 35
          ...
          _tableRowGreen(
              '→ ins Rezept', '${r.forRecipe} g', 'nach Verdopplung'),
          _tableRow(
            '→ behalten',
            '${r.leftover} g',
            r.leftover < 10
                ? '⚠️ wenig – sicher aufbewahren'
                : 'neues Anstellgut',
          ),
        ],
      );
  ...
  TableRow _tableRow(String a, String b, String c) => TableRow(children: [
        _cell(a, AppColors.text2),
        _cell(b, AppColors.gold2, mono: true),
        _cell(c, AppColors.text3, small: true),
      ]);
  TableRow _tableRowBold(String a, String b, String c) => TableRow(children: [
        _cell(a, AppColors.text, bold: true),
        _cell(b, AppColors.text, mono: true),
        _cell(c, AppColors.text3),
      ]);
  TableRow _tableRowGreen(String a, String b, String c) => TableRow(children: [
        _cell(a, AppColors.text2),
        _cell(b, AppColors.green, mono: true),
        _cell(c, AppColors.text3, small: true),
      ]);

  Widget _cell(String t, Color c,
          {bool mono = false, bool bold = false, bool small = false}) =>
      Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: isNow ? AppColors.gold : AppColors.goldDim, width: 2),
              boxShadow: isNow
                  ? [
                      BoxShadow(
...
class _InfoBox extends StatelessWidget {
  ...
  final String text;
  const _InfoBox(
      {required this.color, required this.bgColor, required this.text});
  @override
  Widget build(BuildContext context) => Container(
        padding: const EdgeInsets.all(12),
        ...
        child: Text(text,
            style: TextStyle(fontSize: 12.5, color: color, height: 1.6)),
      );
}
...
class _TempLegend extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _LegendRow('❄️ Kühlschrank (4–6°C):',
              '1–2 Wochen, 1× pro Woche füttern', AppColors.blue),
          _LegendRow(
              '🌿 18–24°C:', 'Ideal — gut vorhersagbar', AppColors.green),
          _LegendRow('🌡️ 28–32°C:', 'Schnell — aufmerksam beobachten',
              Color(0xFFE8C050)),
          _LegendRow('🔥 35°C+:', 'Eiskaltes Wasser + Kühlschrank-Strategie',
              AppColors.orange),
          _LegendRow(
              '☠️ 38°C+:', 'Bakterien sterben — sofort kühlen!', AppColors.red),
        ],
      );
}

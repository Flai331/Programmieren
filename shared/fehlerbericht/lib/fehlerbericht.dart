...
class Fehlerbericht {
  ...
  static void log(String message) {
    ...
    final entry = '[${p2(now.hour)}:${p2(now.minute)}:${p2(now.second)}] $message';
    _log.add(entry);
  ...
  static void logApi(String endpoint, String methode, {int? statusCode, String? fehler}) {
    if (fehler != null) {
  ...
  /// (z. B. `FeedbackService.logDbOperation`) keine Log-Aufrufe verloren
  /// gehen.
  static void logDb(String operation, String tabelle, {String? id, bool erfolgreich = true}) {
  ...
  static void logAuswahl(String feld, String wert, {List<String>? optionen}) {
    final opts = (optionen != null && optionen.isNotEmpty) ? ' [${optionen.join(', ')}]' : '';
  ...
  static void logFehler(String fehler, {String? kontext, StackTrace? stack}) {
    _meldeAutomatisch(fehler, kontext: kontext, stack: stack, art: _artAutoFehler);
  }
  ...
  static void _meldeAutomatisch(
    ...
    final anzeige = (kontext != null && kontext.isNotEmpty) ? '$fehler (Kontext: $kontext)' : fehler;
  ...
  static Future<bool> _sendeManuell({String? beschreibung, Uint8List? screenshot}) {
  ...
  static Future<bool> _versandDurchfuehren({
    ...
    final fingerprint =
        fingerprintOverride ?? _fingerprintBerechnen(fehlerText ?? beschreibung ?? art, seite);
  ...
  static Future<bool> _emailFallback(String text) async {
    ...
    try {
      ...
      log(ok ? 'E-Mail-App geöffnet (Fallback).' : 'E-Mail-App konnte nicht geöffnet werden.');
      return ok;
    } catch (e) {
  ...
  static String _berichtAlsText({
    ...
    b.writeln();
    final tail = protokoll.length > 100 ? protokoll.sublist(protokoll.length - 100) : protokoll;
    b.writeln('PROTOKOLL (letzte ${tail.length} von ${protokoll.length} Einträgen):');
  ...
  try {
    for (var versuch = 1; versuch <= 3; versuch++) {
      try {
        ...
        if (ctx == null) {
          Fehlerbericht.log('Screenshot: RepaintBoundary-Kontext ist null.');
          return null;
        }
        final boundary = ctx.findRenderObject() as RenderRepaintBoundary?;
        if (boundary == null) {
          Fehlerbericht.log('Screenshot: RenderRepaintBoundary nicht gefunden.');
          return null;
        }
        ...
        Fehlerbericht.log('Screenshot erfasst (${(bytes.lengthInBytes / 1024).round()} KB).');
        return bytes;
      } catch (e) {
        ...
        children: [
          ...
          if (Fehlerbericht._buttonEnabled)
            ValueListenableBuilder<bool>(
              ...
              builder: (_, sichtbar, __) =>
                  sichtbar ? const _DraggableBugButton() : const SizedBox.shrink(),
            ),
        ],
      ),
    );
        ...
        child: Material(
          ...
          child: InkWell(
            ...
            onTap: () {
              ...
              if (ctx != null) Fehlerbericht.melden(ctx);
            },
            child: Container(
              width: _groesse,
              height: _groesse,
              ...
              child: const Icon(Icons.bug_report, color: Colors.white70, size: 22),
            ),
          ),
        ),
      ),
  ...
  Future<void> _senden() async {
    ...
    messenger.showSnackBar(
      SnackBar(
        ...
        backgroundColor: erfolg ? Colors.green.shade700 : Colors.orange.shade800,
      ),
    );
  }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text('Was ist passiert?', style: TextStyle(fontSize: 13, color: Colors.grey)),
              ...
              GestureDetector(
                ...
                child: Row(
                  children: [
                    const Text('Protokoll', style: TextStyle(fontSize: 13, color: Colors.grey)),
                    const Spacer(),
...
class _Notion {
  ...
  static const String _token = String.fromEnvironment('NOTION_TOKEN', defaultValue: '');

  // Fest verdrahtete Notion-IDs — bitte nicht verändern, die Struktur
  // ist in Notion bereits angelegt.
  static const String _wurzelSeiteId = '3d185584-dd12-81bb-9516-e34e03839fbc'; // 🐞 Fehlerzentrale
  static const String _registryDbId = '0f07715e-7f9b-4804-adb5-3d0f704febae'; // 📊 Apps
  ...
  static Future<bool> sendeBericht({
    ...
    try {
      final dbId = await _datenbankIdErmitteln(appKey: appKey, appName: appName, version: version);
      ...
      if (kIsWeb) {
        ...
        Fehlerbericht.log('Notion im Web nicht erreichbar (vermutlich CORS-Blockade): $e');
      } else {
  ...
  static Future<String?> _datenbankIdErmitteln({
    ...
    return _appSelbstEinrichten(appKey: appKey, appName: appName, version: version);
  }

  static Future<({String? dbId, String? pageId})> _registryEintragSuchen(String appKey) async {
    ...
    return (dbId: _richTextPlain(props['Datenbank-ID']), pageId: row['id'] as String?);
  }
  ...
  static Future<String?> _appSelbstEinrichten({
    ...
    Fehlerbericht.log('Notion: App "$appKey" unbekannt — richte Fehlerdatenbank ein…');
    ...
    if (pageRes.statusCode != 200) {
      Fehlerbericht.log('Notion: Seite anlegen fehlgeschlagen (HTTP ${pageRes.statusCode})');
      return null;
    }
    final pageId = (jsonDecode(pageRes.body) as Map<String, dynamic>)['id'] as String;
    ...
    if (dbRes.statusCode != 200) {
      Fehlerbericht.log('Notion: Datenbank anlegen fehlgeschlagen (HTTP ${dbRes.statusCode})');
      return null;
    }
    final dbId = (jsonDecode(dbRes.body) as Map<String, dynamic>)['id'] as String;
    ...
    if (regRes.statusCode != 200) {
      ...
      _registryPageIdCache = (jsonDecode(regRes.body) as Map<String, dynamic>)['id'] as String?;
    }

    _dbIdCache = dbId;
    Fehlerbericht.log('Notion: Fehlerdatenbank für "$appName" erfolgreich eingerichtet.');
    return dbId;
  }
  ...
  static Future<bool> _berichtAnlegen({
    ...
    final props = <String, dynamic>{
      ...
      'Art': _selectProp(art),
      if (beschreibung != null && beschreibung.isNotEmpty) 'Beschreibung': _textProp(beschreibung),
      if (fehlerText != null && fehlerText.isNotEmpty) 'Fehler': _textProp(fehlerText),
    ...
    if (protokoll.isNotEmpty) {
      final tail =
          protokoll.length > 100 ? protokoll.sublist(protokoll.length - 100) : protokoll;
      children.add(_headingBlock('Protokoll (${tail.length} von ${protokoll.length} Einträgen)'));
    ...
    final res = await http
        .post(Uri.parse('$_base/pages'), headers: _headers, body: jsonEncode(body))
        .timeout(_timeout);

    if (res.statusCode != 200) {
      Fehlerbericht.log('Notion: Bericht anlegen fehlgeschlagen (HTTP ${res.statusCode})');
      return false;
    }

    final pageId = (jsonDecode(res.body) as Map<String, dynamic>)['id'] as String;
    ...
    if (screenshot != null && !kIsWeb) {
      try {
        ...
        Fehlerbericht.log('Notion: Screenshot-Anhang fehlgeschlagen (ignoriert): $e');
      }
    }
  ...
  static String _titelFuer(String art, String? beschreibung, String? fehlerText, DateTime now) {
    String p2(int n) => n.toString().padLeft(2, '0');
    final zeit = '${p2(now.day)}.${p2(now.month)}. ${p2(now.hour)}:${p2(now.minute)}';
  ...
  static Future<void> _registryAktualisieren({required String appKey, required String version}) async {
    try {
      ...
      try {
        final getRes =
            await http.get(Uri.parse('$_base/pages/$pageId'), headers: _headers).timeout(_timeout);
      ...
      Fehlerbericht.log('Notion: Registry-Update fehlgeschlagen (ignoriert): $e');
    }
  }

  // ── Screenshot-Upload (File-Upload-API) ─────────────────────────────
  static Future<void> _screenshotAnhaengen(String pageId, Uint8List bytes) async {
    // 1) Upload-Objekt anlegen
    final createRes = await http
        .post(
          Uri.parse('$_base/file_uploads'),
          headers: _headers,
          body: jsonEncode({'filename': 'screenshot.png', 'content_type': 'image/png'}),
        )
        .timeout(_timeout);
    if (createRes.statusCode != 200) {
      Fehlerbericht.log('Notion: file_uploads anlegen fehlgeschlagen (HTTP ${createRes.statusCode})');
      return;
    }
    final uploadId = (jsonDecode(createRes.body) as Map<String, dynamic>)['id'] as String;

    // 2) Datei per multipart hochladen
    final req = http.MultipartRequest('POST', Uri.parse('$_base/file_uploads/$uploadId/send'))
      ..headers['Authorization'] = 'Bearer $_token'
      ..headers['Notion-Version'] = '2022-06-28'
      ..files.add(http.MultipartFile.fromBytes('file', bytes, filename: 'screenshot.png'));
    final streamed = await req.send().timeout(const Duration(seconds: 30));
    if (streamed.statusCode != 200) {
      Fehlerbericht.log('Notion: Screenshot-Upload fehlgeschlagen (HTTP ${streamed.statusCode})');
      return;
    }
  ...
  static String? _richTextPlain(dynamic prop) {
    ...
    for (final t in arr) {
      ...
      buf.write(m['plain_text'] ?? (m['text'] as Map<String, dynamic>?)?['content'] ?? '');
    }
    return buf.toString();
  }
